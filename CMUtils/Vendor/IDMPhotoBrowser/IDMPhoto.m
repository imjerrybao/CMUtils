...
+ (IDMPhoto *)photoWithImageId:(NSString *)imageId {
    return [[IDMPhoto alloc] initWithImageId:imageId];
}
...
+ (NSArray *)photosWithImageIds:(NSArray *)imageIdsArray {
    NSMutableArray *photos = [NSMutableArray arrayWithCapacity:imageIdsArray.count];
    
    for (NSString *imageId in imageIdsArray) {
        if ([imageId isKindOfClass:[NSString class]]) {
            IDMPhoto *photo = [IDMPhoto photoWithImageId:imageId];
            [photos addObject:photo];
        }
    }
