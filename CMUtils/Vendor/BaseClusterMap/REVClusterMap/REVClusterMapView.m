...
- (void) setup
{
    ...
    self.blocks = 4;
    self.minVisibleAnnotations = 0;
    
    super.delegate = self;
    zoomSize = self.visibleMapRect.size;
//    zoomLevel = self.visibleMapRect.size.width * self.visibleMapRect.size.height;
//    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
////    button.backgroundColor = [UIColor redColor];
//    [button setImage:LOADIMAGE(@"map_sata_u") forState:UIControlStateNormal];
//    [button setImage:LOADIMAGE(@"map_sata_u") forState:UIControlStateHighlighted];
//    [button setFrame:CGRectMake(self.width - 50, 80, 45, 45)];
//    [button addTarget:self action:@selector(mapMode:) forControlEvents:UIControlEventTouchUpInside];
...
- (void) mapView:(MKMapView *)mapView regionDidChangeAnimated:(BOOL)animated
{
    ...
    if( [self mapViewDidZoom] )
    {
        ...
        NSArray *add = [REVClusterManager clusterAnnotationsForMapView:self forAnnotations:annotationsCopy blocks:self.blocks minClusterLevel:self.minimumClusterLevel minvisibleAnnotations:self.minVisibleAnnotations];
...
- (BOOL) mapViewDidZoom
{
//    double zoomLevelTemp = self.visibleMapRect.size.width * self.visibleMapRect.size.height;//坑爹的精度不一样
...
//    zoomLevel = self.visibleMapRect.size.width * self.visibleMapRect.size.height;
    ...
    return YES;//永远返回YES.移动地图也刷新,但是会刷新掉临时大头针
    
    if( dequals(zoomSize.width,self.visibleMapRect.size.width) && dequals(zoomSize.height,self.visibleMapRect.size.height) )
    {
        return NO;
    }
    zoomSize = self.visibleMapRect.size;
...
- (void) addAnnotations:(NSArray *)annotations
{
    ...
    NSArray *add = [REVClusterManager clusterAnnotationsForMapView:self forAnnotations:annotations blocks:self.blocks minClusterLevel:self.minimumClusterLevel minvisibleAnnotations:self.minVisibleAnnotations];
...
#pragma mark - Action
- (void)mapMode:(UIButton *)button
{
    
//    [button setImage:LOADIMAGE(@"satelite_unselected") forState:UIControlStateNormal];
//    [button setImage:LOADIMAGE(@"satelite_selected") forState:UIControlStateHighlighted];
    if(self.mapType == MKMapTypeStandard)
    {
        [self setMapType:MKMapTypeSatellite];
        [button setImage:LOADIMAGE(@"map_plane_s") forState:UIControlStateNormal];
        [button setImage:LOADIMAGE(@"map_plane_u") forState:UIControlStateHighlighted];
    }
    else
    {
        [self setMapType:MKMapTypeStandard];
        [button setImage:LOADIMAGE(@"map_sata_s") forState:UIControlStateNormal];
        [button setImage:LOADIMAGE(@"map_sata_u") forState:UIControlStateHighlighted];
    }
}
@end
