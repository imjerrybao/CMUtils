...
- (id)initWithHeight:(CGFloat)height
{
    ...
    if (self) {
        ...
        _hintView               = [[UILabel alloc] initWithFrame:self.bounds];
        _hintView.textAlignment = NSTextAlignmentCenter;
        _hintView.font          = [UIFont systemFontOfSize:12];
        _hintView.text          = Locale(@"refreshView.refreshFinish");
        _hintView.textColor     = [UIColor grayColor];
        _hintView.hidden        = YES;
//        [_hintView addObserver:self forKeyPath:@"hidden" options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:nil];
...
- (void)didMoveToSuperview
{
    ...
    if ([self.superview isKindOfClass:[UIScrollView class]])
    {
        ...
        rect.size.width = _scrollView.frame.size.width > SCREEN_WIDTH?SCREEN_WIDTH:_scrollView.frame.size.width;
        self.frame = rect;
...
- (void)restore
{
    ...
    [UIView transitionWithView:_activityIndicatorView
     ...
     } completion:^(BOOL finished)
     {
         ...
         [self performSelector:@selector(restoreHint) withObject:nil afterDelay:1.0];
     }];
...
//{
//    self.superview.userInteractionEnabled = _hintView.hidden;
//}
